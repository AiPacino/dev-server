(function($){
	$.fn.editBox = function(B) {
		...
		var B = $.extend({
			...
			fontsizeoption:["20px","18px","16px","14px","12px"],
			textoption:[
				"收货人{address_name}",
		...
		$.fn.content = function() {
			...
			for(var i = 0; i < _editBox.length; i++){
				...
				_tempText.push('{"width":'+ parseInt(_editItem.css('width')) +',"height":'+ parseInt(_editItem.css('height')) +',"left":'+ parseInt(_editItem.css('left')) +',"top":'+ parseInt(_editItem.css('top')) +',"font_size":"'+ _editItem.find('ul').css('font-size')+'","txt":"'+ _editItem.find('ul').text()+'"}');
			};
		...
		$.fn.addBox = function(C){
			...
			var add = function(){
				...
				_text.css("font-size",C.font_size);
				_this.find("ol").css({
