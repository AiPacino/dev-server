...
(function($){
    ...
    $.message = {
		...
		if(typeof this.elems == "undefined") this.elems = [];
    		var name = 'msg-tip-' + Date.now();
    		this.elems.push(name);
    		$('body').append('<div id="msg-tip" class="hd-load-tips '+ name +'"><div class="load-tips-text">开始请求！</div><div class="load-tips-bg"></div></div>');
    		this.loading();
    	},
    	loading: function(){
    		$('.'+this.elems[0]).children(".load-tips-text").html('处理中，请稍后...');
    	},
    	end: function(msg,second){
	    if( second<1 ){
		second = 1;
	    }
	    var that = this;
	    $('.'+this.elems[0]).children(".load-tips-text").html( msg );
	    setTimeout(function(){
		$('.'+that.elems[0]).animate({opacity: 0},second*1000,function(){
			    $('.'+that.elems[0]).remove();
			    that.elems.splice(0,1);
		    });
	    },300);
	},
	error: function(msg,second){
	    if( second<1 ){
		second = 1;
	    }
	    var that = this;
	    $('.'+this.elems[0]).children(".load-tips-text").html(msg).css({color: 'red'});
	    setTimeout(function(){
		    $('.'+that.elems[0]).animate({opacity: 0},second*1000,function(){
			    $('.'+that.elems[0]).remove();
			    that.elems.splice(0,1);
...
function message(msg,second){
    if( second<1 ){
	second = 1;
    }
    var d = dialog({
	content: '<div class="padding-large-left padding-large-right padding-top padding-bottom bg-white text-small">'+ msg +'</div>'
    });
    d.show();
    setTimeout(function () {
	d.close().remove();
