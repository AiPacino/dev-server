...
class article_control extends init_control {
	...
	public function add(){
		if(checksubmit('dosubmit')){
			if(!empty($_FILES['thumb']['name'])) {
                ...
                $_GET['thumb']  = $result['img']['picturePath'];
			}
            ...
            $channel_list = $this->load->service('channel/channel')->get_list(['status' => 1]);
            $id_arr = array_column($channel_list, 'id');
            $name_arr = array_column($channel_list, 'name');
            $channel_list = array_combine($id_arr, $name_arr);
            $channel_list[0] = '全部';
            ksort($channel_list);
			$this->load->librarys('View')->assign('channel_list', $channel_list)->display('article_edit');
		}
	}
	/**
	 * [edit 编辑文章]
	...
	public function edit(){
		...
		if(checksubmit('dosubmit')){
			...
			$result = $this->service->edit($_GET,true);
			if(!$result){
				showmessage($this->service->error);
			}else{
				showmessage(lang('_operation_success_'),url('index'));
            ...
            $channel_list[0] = '全部';
            ksort($channel_list);
			$this->load->librarys('View')->assign('info',$info)->assign('channel_list', $channel_list)->display('article_edit');
		}
	}
	/**
	 * [delete 删除文章]
