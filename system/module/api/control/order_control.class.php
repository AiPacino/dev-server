...
class order_control extends user_control {
    ...
    public function query(){
        ...
        $spu_goods = $this->spu_serve->api_get_list($spu_where,'id,thumb');
    ...
    public function order_detail($where,$additional){
        ...
        if($result){
            ...
            //如果租期类型按天则重新计算分期金额
            $data['fenqi_amount'] = $order['zuqi_type']==1?sprintf("%.2f",round($order['amount']/$order['zuqi'],2)):sprintf("%.2f",$result[1]['amount']/100);
        }
        ...
        $data['sku_info']   =   array(
            ...
            'market_price'   => $sku_goods['market_price'],
            // 买断金    =  （市场价 * 120%） -  （租金 * 月租金）
