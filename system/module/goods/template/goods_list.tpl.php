<?php include template('header','admin');?>
	...
	<script type="text/javascript" src="./statics/js/admin/order_action.js"></script>
		...
		<div class="content padding-big have-fixed-nav">
			...
			<div class="goods-list-search margin-top clearfix" >
			 ...
			 <input type="hidden" name="label" value="<?php echo $_GET['label'] ? $_GET['label'] : 1?>">
				<div class="top layout margin-bottom fl">
                    ...
                    <div class="form-group form-layout-rank border-none" style="z-index: 2;">
                        <span class="label">按渠道查看</span>
                        <div class="box" style="width: 150px;">
                            <div class="form-select-edit select-search-text-box">
                                <div class="form-buttonedit-popup">
                                    <input class="input" type="text" value="<?php echo $channel['name'] ? $channel['name'] : '渠道选择'?>" readonly="readonly" data-reset="false">
                                    <span class="ico_buttonedit"></span>
                                    <input type="hidden" name="channel_id" value="<?php echo $channel['id']?>" data-reset="false">
                                </div>
                                <div class="select-search-field border border-main">
			...
			<div class="table-work border margin-tb">
				<div class="border border-white tw-wrap">
					<a href="<?php echo url('goods_add')?>"><i class="ico_add"></i>添加</a>
						...
						<?php }?>
					<div class="spacer-gray"></div>
					<a onclick="order_action.push('<?php echo url("ads/admin/ajax_push",array("type"=>"spu")); ?>');" href="javascript:;"><i class="ico_in"></i>推送</a>
                    <div class="spacer-gray"></div>
				</div>
			</div>
			...
			<div class="table resize-table check-table paging-table high-table border clearfix">
				...
				<div class="goods-list">
					...
					<?php foreach ($lists['th'] as $key => $_v) {?>
					<?php 
					$value = $list[$key];
					?>
					...
					<?php }elseif ($lists['th'][$key]['style'] == 'goods') {?>
						<span class="td">
							<div class="td-con td-pic">
							    <div class="pic">
									<!--暂时去掉<img src="<?php echo $list['thumb'] ? $list['thumb'] :'./statics/images/default_no_upload.png';?>" />-->
								</div>
							    <div class="title">
									<div class="double-click">
										<a class="double-click-button margin-none padding-none" title="双击可编辑" href="javascript:;"></a>
					...
					<span class="td">
						<a class="ico_up_rack <?php if($_GET['label'] == 1 || !isset($_GET['label'])){echo 'spu_status';}else{echo 'sku_status'; }?> <?php if($value != 1){echo 'cancel';}?>" href="javascript:;" data-id="<?php echo $list['id']?>" title="点击<?php if($value == 1){?>取消<?php }?>上架"></a>
					</span>
					...
					<span class="td">
						<div class="td-con double-row">
							...
							<?php }else{?>
								<a class="look-attr" target="_blank" data-id="<?php echo $list['id']?>" href="javascript:;">展开</a>
								&nbsp;&nbsp;&nbsp;<a href="<?php echo url('goods_add',array('id' => $list['id']))?>">编辑</a>&nbsp;|&nbsp;
                                      <a href="<?php echo url('ajax_copy',array('spu_id' => $list['id']))?>" data-iframe="true" data-iframe-width="350">复制</a>
					...
					<div class="tr" data-id="<%=item['spu_id']%>">
							...
							<div class="td">
								<div class="td-con">
									<div class="double-click">
										<a class="double-click-button margin-none padding-none" title="双击可编辑" href="javascript:;"></a>
										<input class="input double-click-edit text-ellipsis text-center" name="yajin" data-spuid="<%=item['spu_id']%>" data-id="<%=item['sku_id']%>" type="text" value="<%=item['yajin']%>" />
									</div>
								</div>
							</div>
                        <div class="td">
                            <div class="td-con">
                                <div class="double-click">
                                    <a class="double-click-button margin-none padding-none" title="双击可编辑" href="javascript:;"></a>
                                    <input class="input double-click-edit text-ellipsis text-center" name="buyout_price" data-spuid="<%=item['spu_id']%>" data-id="<%=item['sku_id']%>" type="text" value="<%=item['buyout_price']%>" />
                                </div>
                            </div>
                        </div>
							<div class="td">
		...
		var sure_sku = "<?php echo $lists['sure_sku']?>";
			...
			$(window).load(function(){
				...
				$('[name=yajin]').live('blur',function() {
					var yajin = $(this).val();
					var sku_id = $(this).attr('data-id'),
						spu_id = $(this).attr('data-spuid');
					list_action.change_yajin(ajax_sku,spu_id,sku_id,yajin);
				});
                $('[name=buyout_price]').live('blur',function() {
                    var buyout_price = $(this).val();
                    var sku_id = $(this).attr('data-id'),
                        spu_id = $(this).attr('data-spuid');
                    list_action.change_buyout_price(ajax_sku,spu_id,sku_id,buyout_price);
                });
