...
<div class="content padding-big have-fixed-nav">
    ...
    <!-- 详情设置结束 -->
    <div class="app-actions" style="z-index:999">
...
<script type="text/javascript">
    ...
    function setClass() {
        ...
        top.dialog({
            ...
            onclose: function () {
                if (this.returnValue) {
                    var $checktip = $('#choosecat').nextAll(".validform_checktip");
                    $checktip.addClass("validform_right");
                    $checktip.removeClass("validation-tips");
                    $checktip.html('<i></i>');
        ...
        top.dialog({
            ...
            onclose: function () {
                if (this.returnValue) {
                    ...
                    if($.inArray("1", dataIdArr) == -1 || $.inArray("4", dataIdArr) == -1){
                        alert('成色，租期为必选项');
                        return false;
                    }
                ...
                top.dialog({
                    ...
                    onclose: function(){
                        if(this.returnValue){
                            ...
                            _yajin_change           = this.returnValue[3];
                            _buyout_price_change           = this.returnValue[4];
                            _goods_number_change    = this.returnValue[5];
                            _goods_pre_number_change    = this.returnValue[6];
                            var num_reg = /^[-\+]?\d*$/;
                            var price_reg = /^[-\+]?\d+(\.\d{2})?$/;
                            if(!(num_reg.test(_goods_number_change)) || !(price_reg.test(_shop_price_change))|| !(price_reg.test(_buyout_price_change)) || !(price_reg.test(_market_price_change)) || !(price_reg.test(_yajin_change)) || !(num_reg.test(_goods_pre_number_change))){
                                alert('请输入正确的数字!');
                                return false;
                            }
                            //货号
                            ...
                            //押金
                            $('[attr^="yajin"]').each(function(index,data){
                                num = Number($(this).val()) + Number(_yajin_change);
                                num = num < 0 ? 0 : num;
                                $(this).val(num.toFixed(2));
                            })
                            //买断价格
                            $('[attr^="buyout_price"]').each(function(index,data){
                                num = Number($(this).val()) + Number(_buyout_price_change);
                                num = num < 0 ? 0 : num;
                                $(this).val(num.toFixed(2));
                            })
                            //库存
                            ...
                            //预备库存
                            $('[attr^="pre_number"]').each(function(index,data){
                                num = parseInt($(this).val()) + parseInt(_goods_pre_number_change);
                                num = num < 0 ? 0 : num;
                                $(this).val(num);
                            })
                        }
