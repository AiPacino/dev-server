...
class goods_sku_table extends table {
    ...
    protected $_validate = array(
        array('chengse',0,'{goods/goods_sku_chengse_require}',table::MUST_VALIDATE, 'notequal', table:: MODEL_BOTH),
        array('zuqi',0,'{goods/goods_sku_zuqi_require}',table::MUST_VALIDATE, 'notequal', table:: MODEL_BOTH),
        array('zuqi_type',0,'{goods/goods_sku_zuqi_type_error}',table::MUST_VALIDATE, 'notequal', table:: MODEL_BOTH),
        array('shop_price',0,'{goods/goods_sku_shop_price}',table::MUST_VALIDATE, 'notequal', table:: MODEL_BOTH),
        array('market_price',0,'{goods/goods_sku_market_price}',table::MUST_VALIDATE, 'notequal', table:: MODEL_BOTH),
        array('yajin',0,'{goods/goods_sku_yajin_require}',table::MUST_VALIDATE, 'notequal', table:: MODEL_BOTH),
    );
    ...
    public function detail($id = 0, $field = TRUE) {
        ...
        if (empty($this->result['sku']))
            return $this;
        if ($this->result['sku']['imgs'])
            $this->result['sku']['img_list'] = $this->result['sku']['imgs'];
    ...
    public function spu() {
        ...
        if (!$this->result['sku']['subtitle'])
            unset($this->result['sku']['subtitle'], $this->result['sku']['style']);
        if (!$this->result['sku']['warn_number'])
            unset($this->result['sku']['warn_number']);
        if (!$this->result['sku']['keyword'])
            unset($this->result['sku']['keyword']);
        if (!$this->result['sku']['description'])
            unset($this->result['sku']['description']);
        if (!$this->result['sku']['content'])
            unset($this->result['sku']['content']);
        if ($this->result['sku']['weight'] == 0)
            unset($this->result['sku']['weight']);
        if ($this->result['sku']['volume'] == 0)
            unset($this->result['sku']['volume']);
    ...
    public function channel() {
        if (empty($this->result['goods']))
            $this->result['goods'] = $this->load->table('goods/goods_spu')->find($this->result['sku']['spu_id']);
        $this->result['sku']['channel'] = $this->load->table('channel/channel')->where(array('id' => $this->result['goods']['channel_id']))->find();
        return $this;
    }
    ...
    public function _after_find(&$result, $option = array()) {
        if ($result['spec'])
            $result['spec'] = json_decode($result['spec'], TRUE);
        if ($result['imgs'])
