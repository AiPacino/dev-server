...
class brand_service extends service {
	 ...
	 */
	public function get_lists($page='',$limit=null){
		$result = $this->db->page($page)->limit($limit)->order('sort asc')->getField('id,name,descript,sort,logo,isrecommend,status',TRUE);
		if(!$result){
	...
	public function delete_brand($params){
		...
		$infos = $this->db->where($sqlmap)->getField('logo',true);
