...
class goods_sku_service extends service {
    ...
    public function create_sku($params) {
        if (!empty($params['del'])) {
            ...
            $result = $this->sku_db->where($map)->save(['status' => -1]);
            if($result === false){
        ...
        if (isset($params['new'])) {
            foreach ($params['new'] as $key => $new_data) {
                ...
                $sku_info = $this->sku_db->where(['sn' => $new_data['sn']])->find();
                if($sku_info){
                    $new_data['sku_id'] = $sku_info['sku_id'];
                    $params['new'][$key]['sku_id'] = $this->sku_db->update($new_data);
                }else{
                    $params['new'][$key]['sku_id'] = $this->sku_db->add($new_data);
                }
                if($params['new'][$key]['sku_id'] === false){
                    $this->error = $this->sku_db->getError();
                    return false;
                }
            }
        ...
        if (isset($params['edit'])) {
            foreach ($params['edit'] AS $edit_data) {
                ...
                $result = $this->sku_db->update($edit_data);
                if($result === false){
                    $this->error = $this->sku_db->getError();
                    return $result;
                }
    ...
    public function sku_edit($params) {
        ...
        $result = $this->sku_db->save($params);
            ...
            if ($label == 4) {
                ...
                foreach ($spu_ids AS $spu_id) {
                    ...
                    if ($sku_status_num == count($sku_status)) {
                        ...
                        //逻辑删除商品轮播图数据
                        $adv_info = $this->adv_db->where(array('content_id' => $spu_id))->field('id,position_id,title,content,content_id,filag as flag,link')->select();
                        if($adv_info){
                            $status['status'] = 0;
...
//        if (!empty($params['zuqi'])) {
//            $min_max_price = $this->sku_db->field("min(zuqi) AS min_month, max(zuqi) AS max_month")->where(array("spu_id" => $params['spu_id']))->find();
//            $this->spu_db->where(array('id' => $params['spu_id']))->save($min_max_price);
//        }
