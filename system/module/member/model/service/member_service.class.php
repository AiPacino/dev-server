...
//     }
    ...
    public function register($params = array()) {
        ...
        foreach ($params as $k => $v) {
            $method = '_valid_' . $k;
            if ($k == 'vcode') {
                if (method_exists($this, $method) && $this->$method($v, $params['mobile']) === false)
                    return false;
            }else {
    ...
    public function action_frozen($mid, $money, $isfrozen = true, $msg = '') {
        if ($isfrozen === true) {
            ...
            if (!$result)
                $this->model->where(array('id' => $mid))->setDec('frozen_money', $money);
        } else {
    ...
    public function resetmobile($params, $mid) {
        ...
        if (!$this->_valid_mobile($params['mobile']))
            return false;
        if ($mobile_validate) {
            extract($params, EXTR_IF_EXISTS);
            $sqlmap = $data = array();
     ...
     * 获取 认证平台列表
     */
    public function get_certification_platform_list(){
	return [
	    1 => '芝麻认证',
	];
    }
     ...
     * @return string
     */
    public function get_certification_paltform_name( $paltform ){
	$list = zuji\certification\Certification::getPlatformList();
	//$list = $this->get_certification_platform_list();
	if( isset($list[$paltform]) ){
	    return $list[$paltform];
	}
	return '--';
    }
    ...
    public function get_lists($sqlmap, $page, $limit) {
        $members = $this->load
		->table('member/member')
		->field('id,username,password,encrypt,mobile,email,certified,face,risk,certified_platform,credit,cert_no,realname,credit_time,islock,block,login_ip,login_num,login_time,register_time,withholding_no')
		->where($sqlmap)
		->page($page)
		->order('id DESC')
		->limit($limit)
        ...
        foreach ($members AS $member) {
            $lists[] = array(
                ...
                'block' => $member['block'],
                'order_remark' => $member['order_remark'],
                'exp' => $member['exp'],
                ...
                'face' => $member['face'],
                'face_show' => $member['face']==1?'通过':'--',
                'risk' => $member['risk'],
                'risk_show' => $member['risk']==1?'通过':'--',
                'certified' => $member['certified'],
                'certified_show' => $member['certified']==1?'已认证':'--',
                'certified_platform' => $member['certified_platform'],
                'certified_platform_name' => $this->get_certification_paltform_name($member['certified_platform']),
                'credit' => $member['credit'],
                'cert_no' => $member['cert_no'],
                'realname' => $member['realname'],
                'credit_time' => $member['credit_time'],
                'credit_time_show' => $member['credit_time']>0?date('Y-m-d H:i:s', $member['credit_time']):'--',
                'withholding_no' => $member['withholding_no'],
            );
        }
        return $lists;
    }
    ...
    /**
     * 解封下单
     * @param $member_id 【必须】int 会员ID
     * @return boolean true:成功 false:失败
     */
    public function on_deblocking($member_id){
        if($member_id<1){
            set_error("会员ID错误");
            return false;
        }
        ...
        return $this->model->deblocking($member_id,$data);
     ...
     * @return boolean true:成功 false:失败
     */
    public function off_deblocking($member_id){
        if($member_id<1){
            set_error("会员ID错误");
            return false;
        }
     ...
     * @param $data array
     * [
     *      'member_id' => '',	      //【必须】  int 会员ID
     *      'admin_id' =>'',          //【必须】  int 操作员ID
     *      'admin_remark' => '',	  //【必须】  操作员备注
     * ]
     * @return mixed	false：创建失败；int:主键；创建成功
     * @author wuhaiyan<wuhaiyan@huishouhao.com.cn>
     */
    
    public function create_deblocking($data){
        $data = filter_array($data, [
            'member_id' => 'required|is_id',
        ...
        $data['deblocking_time'] =time();
        $this->block = $this->load->table('member/member_deblocking');
        
        $follow_id = $this->block->create_block($data);
        return $follow_id;
    }
     ...
     * @return mixed	false：查询失败；array：解封记录信息
     * [
     *      'block_id'=>'',           //【必须】  int 主键ID
     *      'member_id' => '',	      //【必须】  int 会员ID
     *      'admin_id' =>'',          //【必须】  int 操作员ID
     *      'admin_remark' => '',	  //【必须】  string 操作员备注
     *      'deblocking_time' => '',  //【必须】 int 解封时间
     * ]
     * @author wuhaiyan<wuhaiyan@huishouhao.com.cn>
     */
    public function get_by_member_id($member_id,$additional=[]) {
        ...
        $this->block = $this->load->table('member/member_deblocking');
        $block_info= $this->block->get_by_member_id($member_id,$additional);
        return $block_info;
    }
    public function get_count($member_id){
        ...
        $this->block = $this->load->table('member/member_deblocking');
        return $this->block->where(['member_id'=>$member_id])->count();
    }
