...
class member_address_service extends service
{
	...
	public function _initialize(){
		$this->address = $this->load->table('member/member_address');
        $this->district = $this->load->service('admin/district');
	}
     ...
     * @author limin
     */
    public function user_address_default($userid){
        $field = "id,mid,name,mobile,address,district_id,zipcode";
        $rom = $this->address->user_address($userid,$field);
        if($rom){
            //查询区
            $country = $this->district->fetch_by_id($rom['district_id']);
            //查询市
            $city    = $this->district->fetch_by_id($country['parent_id']);
            //查询市
            $provin    = $this->district->fetch_by_id($city['parent_id']);

            $rom['provin_id']         = $provin['id'];
            $rom['provin_name']   = $provin['name'];
            $rom['city_id']             = $city['id'];
            $rom['city_name']       = $city['name'];
            $rom['country_id']      = $country['id'];
            $rom['country_name'] = $country['name'];
            return $rom;
        }
        return false;
    }
	...
	public function lists($sqlmap = array(), $limit = 20, $page = 1) {
        ...
        $count = $this->address->where($this->sqlmap)->count();
	...
	public function add($params = array()) {
		...
		/* 校验发布条数 */
		$count = $this->address->where(array('mid' => $params['mid']))->count();
		if($count == 20) {
		...
		$result = $this->address->update($params);

		$id = $params['id'] ? $params['id'] : $result;
		if(!empty($params['default'])){
			$this->address->where(array("id" => array("NEQ", $id), 'mid' => $params['mid']))->setField('isdefault', 0);
		}
		if($result === false) {
	...
	public function edit($params = array()) {
		...
		if(!empty($params['default'])) {
			$params['isdefault'] = 1;
			$this->address->where(array("id" => array("NEQ", $params['id']), 'mid' => $params['mid']))->setField('isdefault', 0);
		}
	...
	public function set_default($id, $mid) {
		$result = $this->address->where(array("id" => $id))->setField('isdefault', 1);
		if(!$result) {
			$this->error = lang('该地址不存在');
			return false;
		}
		$this->address->where(array("id" => array("NEQ", $id), 'mid' => $mid))->setField('isdefault', 0);
		return true;
	}

	public function fetch_by_id($id) {
		$this->sqlmap['id'] = $id;
		$r = $this->address->where($this->sqlmap)->find();
		if(!$r) {
			$this->error = $this->address->getError();
			return false;
		}
		...
		list($cid,$r['province_id'],$r['city_id'],$r['country_id']) = $r['district_tree'];
		list($cname,$r['province_name'],$r['city_name'],$r['country_name']) = $r['full_district'];
		return $r;
	}
