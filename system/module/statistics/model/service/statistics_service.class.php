...
class statistics_service extends service {
	...
	public function build_sqlmap($params = array()) {
        ...
        if(isset($params['month']) && $params['month'] > 0){
            $months = $params['month'];
            $months -= 1;
            $this->where['search']['month_time'] = array('BETWEEN',array(strtotime("-{$params['month']}month",strtotime(date('Y-m-1 00:00:00'))) ,time()));
	...
	public function sales() {
        ...
        $member_table=$this->member_table->getTableName();
		...
		if ($this->where['search']) {
			...
			$order_field = "FROM_UNIXTIME({$order_table}.create_time,'%m月%d日') days,count({$order_table}.order_id) orders";
			...
			$member_sqlmap[$member_table.'.register_time'] = $search;
			$regist_field = "FROM_UNIXTIME({$member_table}.register_time,'%m月%d日') days,count({$member_table}.id) ids";
			$regist_searchs = $this->member_table->where($member_sqlmap)->field($regist_field)->group('days')->select();
			
			$month_sqlmap[$order_table.'.create_time'] = $months_search;
			$month_sqlmap[$order_table.'.payment_status'] = \zuji\order\PaymentStatus::PaymentSuccessful;
			$month_field = "FROM_UNIXTIME({$order_table}.create_time,'%m月') months,count({$order_table}.order_id) orders";
			$month_searchs =$this->db_order->where($order_sqlmap)->field($month_field)->group('months')->select();
...
//			var_dump($payment_searchs);die;
			foreach ($order_searchs as $k => $val) {
			    $orders[$val['days']] = $val['orders'];   
			}
			foreach ($payment_searchs as $k => $val) {
			    $payments[$val['days']] = $val['orders'];
			}
			
			foreach ($regist_searchs as $k => $val) {
			    $regists[$val['days']] = $val['ids'];
			}
			foreach ($month_searchs as $k => $val) {
			    $_months[$val['months']] = $val['orders'];
			}
			...
			for ($i = 0; $i <= $days; $i++) {
			    ...
			    $_orders[] = isset($orders[$today])?$orders[$today]:0;
			    $_payments[] = isset($payments[$today])?$payments[$today]:0;
			    $_regists[] = isset($regists[$today])?$regists[$today]:0;
			    $this->result['search']['dates'][$i] = $today;
			    	    
			}
			
			for($i =0;$i<=$months;$i++){
			    $month = date('m月',strtotime("+{$i}month",$months_search[1][0]));
			    $month_orders[] =isset($_months[$month])?$_months[$month]:0;
			    $this->result['search']['months'][$i] = $month;
			}

			$this->result['search']['orders'] = $_orders;
			$this->result['search']['payments'] = $_payments;
			$this->result['search']['regists'] = $_regists;
			$this->result['search']['months_orders'] = $month_orders;
		...
		$thirty = array('BETWEEN',array($start ,$end));
		//近 7日 查询条件
		$start = time()-7*86400;
		$end   = time();
        ...
        $login_users = $this->load->service('member2/member')->get_list(['login_time'=>$seven]);
        ...
        //近30日成交机型TOP10
        $orders =$this->load->service('order2/order')->get_order_list(
            ['create_time'=>$thirty,
             'payment_status' =>\zuji\order\PaymentStatus::PaymentSuccessful,
            ],
            ['goods_info'=>true,'size'=>'all']);
        for($i=0;$i<count($orders);$i++){
            $spu_id[]= $orders[$i]['goods_info']['spu_id'];     
        }
      
        $pai =array_count_values($spu_id);
        $machine_list = [];
        foreach ($pai as $k => $v){
            $spu_info = model('goods/goods_spu')->where(['id' => $k])->find();
       
            $machine_list[$spu_info['machine_id']][$k] = $v;
        }
        foreach ($machine_list as $k => $v){
            $machine_list[$k] = array_sum(array_values($v));
        }
        arsort($machine_list);
        $a =array_slice($machine_list,0,10,true);
        $machine_service =$this->load->service('goods/goods_machine_model');
        $machine_name =[];
        $machine_number=[];
        asort($a);
        foreach ($a as $k=>$v){
            $machine = $machine_service->get_info($k);
           $machine_name[] =$machine['name'];
           $machine_number[] =$v;
        }
        
        
        $this->result['search']['machine']['number']   = $machine_number;
        $this->result['search']['machine']['name']   = $machine_name;
