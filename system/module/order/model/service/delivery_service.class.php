...
class delivery_service extends service {
    ...
    public function get_api_list($field = '',$where = array()) {
        ...
        return $this->table->get_api_list($field,$where);
    }
	/**
	 * 获取物流列表
	 */
	 ...
	 * @return [result]
	 */
	public function delete_delivery($o_d_id) {
		$o_d_id = (int) remove_xss($o_d_id);
		$result = $this->table_delivery->find($o_d_id);
		if (!$result) {
			$this->error = $this->table_delivery->getError();
		...
		$skuids = explode(',',$result['o_sku_ids']);
		$this->load->table('order/order_sku')->where(array('id'=>array('IN',$skuids)))->save(array('delivery_id'=>0,'delivery_status'=>0));
		$result = $this->table_delivery->where(array('id'=>$o_d_id))->delete();
		if($result === false){
			$this->error = $this->table_delivery->getError();
			return FALSE;
		}
		// 更新快递100数据
		$this->load->table('order/order_track')->where(array('delivery_id'=>$o_d_id))->delete();
		return $result;
	}
