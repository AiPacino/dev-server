                ...
                <div class="tr">
                    <?php foreach ($lists['th'] AS $k=>$th) {?>
                            ...
                            <?php }elseif ($lists['th'][$k]['style'] == 'ico_up_rack') {?>
                                <span class="td">
						        <a class="ico_up_rack <?php if($item['is_open'] != 1){?>cancel<?php }?>" href="javascript:;" data-id="<?php echo $item['id']?>" title="点击启用，禁用"></a>
					            </span>
                            <?php }else{?>
                                <span class="td">
...
<script>
    ...
    $(window).load(function(){
        ...
        $('input[name=credit_name]').bind('blur',function() {
            var name = $(this).val();
            var id = $(this).attr('data-id');
