<?php include template('header','admin');?>
	<body>
		...
		<div class="content padding-big have-fixed-nav">
			...
			<div class="author-wrapper padding">
				...
				<dl class="aut-line clearfix">
					...
					<dd>
                        <div style="width:300px;" class="clearfix aut-box">
                            <div class="clearfix">
                                <label><input class="aut-input aut-two-input" type="checkbox" name="rules[]" value="<?php echo $v1['id']?>" <?php echo $checked?> /><?php echo $v1['name']?></label>
                            </div>
                            <?php foreach ($v1['_child'] as $k2=>$v2):?>
                                <?php $checked=in_array($v2['id'],$data['rules'])?'checked':''?>
                                <div  style="margin-left: 80px;">
                                    <label><input class="aut-input sub-aut-input" type="checkbox" name="rules[]" value="<?php echo $v2['id']?>" <?php echo $checked?> /><?php echo $v2['name']?></label>
                                </div >
                            <?php endforeach;?>
                        </div>
					</dd>
		...
		<script>
			...
			//二级总按钮
			$(".aut-two-input").click(function () {
                if($(this).is(":checked")){
                    $(this).parents(".aut-box").find(".aut-input").each(function(){
                        $(this).attr("checked",true);
                    });
                }else{
                    $(this).parents(".aut-box").find(".aut-input").each(function(){
                ...
                var _parent = $(this).parents(".aut-line").find('dt input');
                _parent.attr('checked',true);

                selectAll();
            });
			//单个按钮
			$(".sub-aut-input").click(function(){
				if($(this).is(":checked")){
					$("#check-aut-all").attr("checked",false);
				}
				var _parent = $(this).parents(".aut-line").find('dt input');
                _parent.attr('checked',true);
				var parent2 = $(this).parents(".aut-box").find('.aut-two-input');
				if(parent2.length){
				    parent2.attr('checked',true);
                }
