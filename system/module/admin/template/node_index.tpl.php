<?php include template('header','admin');?>
		<div class="fixed-nav layout">
			<ul>
				<li class="first">节点管理<a id="addHome" title="添加到首页快捷菜单">[+]</a></li>
		...
		<div class="content padding-big have-fixed-nav">
			<div class="table-work border margin-tb">
				<div class="border border-white tw-wrap">
					<a href="<?php echo url('add') ?>" data-iframe="true" data-iframe-width="320" data-iframe-title="添加节点"><i class="ico_add"></i>添加</a>
					...
					<?php }elseif ($lists['th'][$key]['style'] == 'data') {?>
						<div class="td">
							<div class="tree-edit-input">
								...
								<a class="tree-add-button" href="<?php echo url('add', array('parent_id' => $list['id'])) ?>" data-iframe="true" data-iframe-width="320"><em class="ico_add"></em>添加子节点</a>
							</div>
						</div>
...
$(window).load(function(){
    ...
    $(".table .ico_up_rack").bind('click',function(){
        if(ajax_status($(this).attr('data-id'))){
	            ...
	            success: function(data){
	            	...
	            	$.each(data, function(i,obj) {
	            		html += '<div class="tr'+(flog?' selected':'')+'" data-tree-id="'+tr.attr("data-tree-id")+'-'+(i+1)+'" data-tree-parent-id="'+tr.attr("data-tree-id")+'" data-id="'+obj.id+'" style="visibility: visible;">'
							...
							+'			</div>'
							+(obj.level==3?'':'<a class="tree-add-button" href="?m=admin&c=node&a=add&parent_id='+ obj.id +'" data-iframe="true" data-iframe-width="320"><em class="ico_add"></em>添加子节点</a>')
							+'		</div>'
							+'	</div>'
							+'	<div class="td">'
							+'		<a href="?m=admin&c=node&a=edit&id='+ obj.id +'" data-iframe="true" data-iframe-width="320">编辑</a>&nbsp;&nbsp;&nbsp;<a href="?m=admin&c=node&a=delete&ids[]='+ obj.id +'" class="delete">删除</a>'
							+'	</div>'
							+'</div>';
	...
	$(".tree-indenter .input").live('change',function(){
		...
		$.post("index.php?m=admin&c=node&a=ajax_sort",data,function(ret){
			message(ret.message);
		},'json');
	});
	...
	$(".tree-edit-input .input").live('change',function(){
		...
		$.post("index.php?m=admin&c=node&a=ajax_name",data,function(ret){
			message(ret.message);
		},'json');
	});
	...
	$(".tr .delete").live('click',function(){
		...
		var d = dialog({
		    title: '提示',
		    content: '<p class="layout text-center padding-big bg-white">确认删除所选节点？删除选择节点，其所有下属节点也将会同步删除且不可恢复。</p>',
		    width: "300",
		    okValue: '确定',
		    ok: function () {
		        $.post("index.php?m=admin&c=node&a=delete",data,function(ret){
					message(ret.message);
				},'json');
		    },
		    cancelValue: '取消',
	...
	$("#delete-area").click(function(){
		...
		var d = dialog({
		    ...
		    okValue: '确定',
		    ok: function () {
		        $.post("index.php?m=admin&c=node&a=delete",{'ids[]':data},function(ret){
					message(ret.message);
				},'json');
		    },
		    cancelValue: '取消',
