<?php
namespace alipay\zmop;

use alipay\zmop\RSAUtil;
use alipay\zmop\WebUtil;
...
class ZmopClient {
	...
	public function execute($request) {
		...
		if (isset ($bizRespObject->error_code)) {
		    // 自定义记录错误方式（debug）
		    \zuji\debug\Debug::error(\zuji\debug\Location::L_Certification, '读取[芝麻认证]结果失败', [
			'url' => $requestUrl,
			'request' => $encryptedApiParams,
...
//				));
	...
	private function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		// 自定义记录错误方式（debug）
		\zuji\debug\Debug::error(\zuji\debug\Location::L_Certification, '[芝麻认证]错误', $logData);
