...
class AopClient {
    ...
    public function alonersaSign($data, $privatekey, $signType = "RSA", $keyfromfile = false) {
        ...
        if (!$keyfromfile) {
            $priKey = $privatekey;
            //writeLog('alonersaSign() privatekey: ' . $priKey);
            $res = "-----BEGIN RSA PRIVATE KEY-----\n" .
                    wordwrap($priKey, 64, "\n", true) .
                    "\n-----END RSA PRIVATE KEY-----";
            //writeLog('alonersaSign() RSA PRIVATE KEY: ' . "\n" . $res);
        } else {
        ...
        if ("RSA2" == $signType) {
            $b = openssl_sign($data, $sign, $res, OPENSSL_ALGO_SHA256);
        } else {
            $b = openssl_sign($data, $sign, $res);
        }
        if (!$b) {
            //writeLog('openssl_sign() ->  false ');
        }
	...
	// 自定义记录错误方式（debug）
	\zuji\debug\Debug::error(\zuji\debug\Location::L_Certification, '[支付宝Aop]错误', $logData);
...
//        $logger->log($logData);
    ...
    public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
        ...
        //发起HTTP请求
        try {
            //writeLog('request: $sysParams: ' . var_export($sysParams, true));
            //writeLog('request: $apiParams: ' . var_export($apiParams, true));
        ...
        if (false === $respWellFormed) {
			// 自定义记录错误方式（debug）
			\zuji\debug\Debug::error(\zuji\debug\Location::L_Certification, '[支付宝Aop]错误--HTTP文本不是标准JSON或者XML', [
				'format' => $this->format,
				'postCharset' => $this->postCharset,
				'fileCharset' => $this->fileCharset,
				'resp' => $resp,
				'r' => $r,
				'respObject' => json_encode($respObject),
			]);
...
//        writeLog('$signType: ' . $signType);

        if ($this->checkEmpty($this->alipayPublicKey)) {

            $pubKey = $this->alipayrsaPublicKey;
            //writeLog('alipayrsaPublicKey: ' . $pubKey);
            $res = "-----BEGIN PUBLIC KEY-----\n" .
                    wordwrap($pubKey, 64, "\n", true) .
                    "\n-----END PUBLIC KEY-----";
            //writeLog('alipay PUBLIC KEY: ' . "\n" . $res);
        } else {
            //读取公钥文件
        ...
        if(!$res){
	    set_error('支付宝RSA公钥错误。请检查公钥文件格式是否正确');
	    return false;
	}
