...
class table extends hd_base {
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            ...
            if(config('DB_FIELDS_CACHE')) {
                ...
                $fields = cache('fields/'.strtolower($db.'.'.$this->name));
                if($fields) {
                    $version    =   config('DB_FIELD_VERSION');
    ...
    public function flush() {
        ...
        if(config('DB_FIELD_VERSION')) $this->fields['_version'] =   config('DB_FIELD_VERSION');
        if(config('DB_FIELDS_CACHE')){
            $db   =  $this->dbName?$this->dbName:config('DB_NAME');
            cache('fields/'.strtolower($db.'.'.$this->name),$this->fields);
        }
    }
    ...
    public function switchModel($type,$vars=array()) {
        ...
        if(!class_exists($class))
            throw_exception($class.lang('_model_not_exist_'));
        // 实例化扩展模型
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),$this->methods,true)) {
            ...
            return $this->getField(strtoupper($method).'('.$field.') AS hd_'.$method);
            ...
            throw new \Exception(__CLASS__.':'.$method.lang('_method_not_exist_'));
            return;
        }
    }
    // 回调方法 初始化模型
    ...
    public function add($data='',$options=array(),$replace=false) {
        if(empty($data)) {
            ...
            if(!empty($this->data)) {
                ...
                $this->error    = lang('_data_type_invalid_');
                return false;
            }
        }
        // 分析表达式
    ...
    public function selectAdd($fields='',$table='',$options=array()) {
        ...
        if(false === $result = $this->db->selectInsert($fields?$fields:$options['field'],$table?$table:$this->getTableName(),$options)){
            // 数据库插入操作失败
            $this->error = lang('_operation_wrong_');
            return false;
        }else {
            // 插入成功
            return $result;
    ...
    public function save($data='',$options=array()) {
        ...
        // return $this->db->getLastSql();exit;
        if(false !== $result) {
    ...
    protected function _parseOptions($options=array()) {
        ...
        // 字段类型验证（ where 是数组，并且 fields存在，而且咩有定义 json ）
        if(isset($options['where']) && is_array($options['where']) && !empty($fields) && !isset($options['join'])) {
            // 对数组查询条件进行字段类型检查
            foreach ($options['where'] as $key=>$val){
                $key            =   trim($key);
                if(in_array($key,$fields,true)){// 检测查询字段，在字段集合中（校验数据类型）
                    if(is_scalar($val)) {// 判断值是否为标量
                        $this->_parseType($options['where'],$key);// 标量值，校验数据类型
                    }
                // 非数字，非'_'开头，不包含'.' '(' '|' '&'  ,会被 unset 掉
     ...
     * @return mixed  null|false 时为失败； array： 为成功
     */
            ...
            foreach($_validate as $key=>$val) {
                ...
                if(empty($val[5]) || $val[5]== self::MODEL_BOTH || $val[5]== $type ) {
                    if(0==strpos($val[2],'{') && strpos($val[2],'}'))
                        // 支持提示信息的多语言 使用 {语言变量名} 方式
                        $_vail = substr($val[2],1,-1);
                        if(strpos($_vail,'/')){
                            list($_module,$_lang) = explode('/',$_vail);
                            $val[2]  =  lang($_lang,$_module.'/language');
                        }else{
                            $val[2]  =  lang($_lang,$_module.'/language');
                        }
    ...
    protected function parseSql($sql,$parse) {
        ...
        if(true === $parse) {
            ...
            $sql    =   strtr($sql,array('__TABLE__'=>$this->getTableName(),'__PREFIX__'=>config('DB_PREFIX')));
        }
        $this->db->setModel($this->name);
        return $sql;
    }
    ...
    public function data($data=''){
        ...
        if(is_object($data)){
            ...
            throw_exception(lang('_data_type_invalid_'));
        }
        $this->data = $data;
        return $this;
    }
     ...
     * @return Model
     */
    public function cache($key=true,$expire=null,$type='file'){
        if(false !== $key)
