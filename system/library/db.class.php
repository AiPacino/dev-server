...
class db
{
    ...
    public function factory($db_config = '') {
        ...
        require_cache(LIB_PATH . 'db/' . $class . EXT);
        // 检查驱动类
    ...
    protected function debug() {
        ...
        $this->model = '_hd_';
	    debug($this->_linkID->host_info.'-SQL: ', $this->queryStr);
    }
     ...
     * @access protected
     */
    protected function debug_transaction($str='') {
        $this->modelSql[$this->model] = $this->queryStr;
        $this->model = '_hd_';
        if($str){
            debug_transaction($this->_linkID->host_info.'-transaction: ', $str);
        }else{
            debug_transaction($this->_linkID->host_info.'-transaction: ', $this->queryStr);
        }
    }
    ...
    protected function parseJoin($join) {
        ...
        $joinStr = preg_replace_callback("/__([A-Z_-]+)__/sU", function($r){return config("DB_PREFIX") . ".strtolower('". $r[1] ."')";}, $joinStr);
        return $joinStr;
    }
