...
class Yidun{
	...
	/**
	 * 获取蚁盾结果是否有风险
	 * @return boolen true：有风险；false：无风险
	 */
	...
	private function __check_risk($data) {
		...
		if( !$this->__parse_data($data)){
			...
			$this->flag = false;
			Debug::error(Location::L_Certification, '信用认证过程蚁盾请求参数错误', ['data'=> get_error(),'params'=>$data]);
			return false;
		}
		//调用curl进行post请求
		$headers = array(
		...
		if( !$result || !isset($result['code']) || $result['code'] != 'OK' ) {
			Debug::error(Location::L_Certification, '信用认证蚁盾数据获取失败', ['data'=> get_error(),'params'=>$data,'result'=>$result]);
		}
		//正常的$result的参数信息
		/*
		 $result = {
