...
class db_mysqli extends db{
    ...
    public function __construct($config=''){
        if ( !extension_loaded('mysqli') ) {
            throw new Exception(lang('_NOT_SUPPERT_').':mysqli');
        }
        if(!empty($config)) {
            $this->config   =   $config;
     ...
     * @access public
     * @param string $str  sql指令
     * @return mixed  sql执行失败是，返回false；sql执行成功时，返回受影响的记录数
     */
    ...
    public function startTrans() {
        ...
        if ($this->transTimes == 0) {
            $n = $this->_linkID->autocommit(false);
            if( !$n ){
                $this->debug_transaction('------------begin_transaction_error------------');
                return false;
            }
            // 开启事务
            $n = $this->_linkID->begin_transaction(  );
            if( !$n ){
                $this->debug_transaction('------------begin_transaction_error------------');
                return false;
            }
        ...
        $this->transTimes++;
        $this->debug_transaction('------------begin_transaction------------');
        return true;
    }
    ...
    public function commit() {
        if ($this->transTimes > 0) {
            ...
            if(!$result){
                $this->error();
                $this->debug_transaction('------------commit_transaction_error:'.$this->error().'------------');
                return false;
            }
        }
        $this->debug_transaction('------------commit_transaction------------');
        return true;
    }
    ...
    public function rollback() {
        if ($this->transTimes > 0) {
            ...
            if(!$result){
                $this->error();
                $this->debug_transaction('------------rollback_transaction_error:'.$this->error().'------------');
                return false;
            }
        }
        $this->debug_transaction('------------rollback_transaction------------');
        return true;
    }
	...
	// 数据库插入错误，导致循环执行
//	\zuji\debug\Debug::error(zuji\debug\Location::L_DB, 'SQL错误', [
//	    'SQL' => $this->queryStr,
//	    'errno' => $this->_linkID->errno,
//	    'error' => $this->_linkID->error,
//	]);
	debug('SQL Error: ', $this->error);
        return $this->error;
    }
