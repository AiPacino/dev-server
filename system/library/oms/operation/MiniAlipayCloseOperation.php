 ...
 *
 */
class MiniAlipayCloseOperation implements OrderOperation
{
    ...
    public function update(){  
        ...
        $order_data = [
            ...
            'refund_amount' => $this->Order->get_yajin(),
            'refund_time' => time(),
        ...
        $instalment_data= [
            'status'=>Instalment::CANCEL,
        ];
        $b = $instalment_table->where(['order_id'=>$order_id,'status'=>Instalment::UNPAID])->save($instalment_data);
        if($b===false){
            set_error("取消分期失败");
            return false;
        }
