...
class form {
    ...
    static public function input($type = 'text', $name, $value, $label, $description='', $attribute = array()) {
		...
		if($type == 'hidden'){
	        ...
	        $string = '<div class="form-group '.$attribute['css'].'"><span class="label">'.$label.'</span>';
    ...
    static public function textarea($name, $value, $attribute = array()) {
        extract($attribute);
        return '<textarea class="textarea hd-input" id = "'.$name.' "name="'.$name.'" placeholder="'. $placeholder .'" '.self::_buildAttribute($attribute).'>'.$value.'</textarea>';
    }
