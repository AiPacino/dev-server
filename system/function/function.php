...
/**
 * 获取、设置 或 加载 配置参数（CONF_PATH 目录中）
 * config('name')   // 直接获取配置参数
 * config(null, 'config'); // 只加载 配置文件config.php中的配置，不考虑返回值
 * config('name','wap') // 从 配置目录的wap.php文件加载配置参数，并且返回 'name' 的值
 * config('name',null,'value') // 设置 参数 'name' 的值为 'value'
 * @param string $name 配置变量
 * @param string $file 配置所在的文件名称
...
function page_url($param = array(), $url=null) {
 ...
 * 生成租机协议号规则
 */
function build_protocol_no($suffix = '') {
    return $suffix.date('Ymd').substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
}
...
function debug( $msg, $data ){
    if( APP_DEBUG ){
        $str = date('Y-m-d H:i:s').' [DEBUG] '.$msg.' '.var_export($data,true);
        $dir = DOC_ROOT.'/data/'.'debug-'.date('Y-m');
        if (!file_exists($dir)){
            mkdir ($dir,0777,true);
        }
        $filename = $dir.'/debug-'.date('d-H').'.log';
 ...
 * @param mixed $data   数据
 */
function debug_transaction( $msg, $data ){
    if( APP_DEBUG ){
        $str = date('Y-m-d H:i:s').' [DEBUG] '.$msg.' '.var_export($data,true);
        $dir = DOC_ROOT.'/data/'.'debug-transaction-'.date('Y-m');
        if (!file_exists($dir)){
            mkdir ($dir,0777,true);
        }
        $filename = $dir.'/'.session_id().'-debug-'.date('d-H').'.log';
        file_put_contents($filename, $str."\n", FILE_APPEND|LOCK_EX);
    }
}
...
/**
 * 混合的 数组trim() 方法
 * 1）支持 string，数字，或 string和int组成的数据
 * 2）其他类型的值，不做任何处理
 * @param array	    $data		【必选】待处理的数组
 * @param string    $character_mask	【可选】要去除的字符集合，默认为" \t\n\r\0\x0B"（所有空白符）
 * @return mixed	返回值类型与$data类型一致
 */
function mixed_trim( &$data, $character_mask = " \t\n\r\0\x0B" ){
    // 数组类型时
    if( is_array( $data ) ){
	$arr = [];
	foreach ($data as $key => $value) {
	    $arr[$key] = mixed_trim($value,$character_mask);
	}
	return $arr;
    }
    elseif( is_string($data) ){
	return trim($data,$character_mask);
    }
    return $data;
}
...
/**
 * 混合的 map() 方法
 * @param mixed		$data	    【必须】待 map 数据
 * @param callable	$callable   【必须】map 函数
 * @return mixed	返回值类型与$data类型一致
 */
function mixed_map( &$data, callable $callable ){
    // 数组类型时
    if( is_array( $data ) ){
	$arr = [];
	foreach ($data as $key => $value) {
	    $arr[$key] = mixed_map($value, $callable);
	}
	return $arr;
    }
    // 非数组类型时，直接执行
...
/**
 * 二维数组合并
 * 将两个二维数组，根据指定$key键的值，从第二个数组中找到对应的数组，添加到第一个数组中对应的数组中
 * 【注意：】该方法会修改第一个数组。
 * 合并方式由两种：
 * 1）当$outkey=null时，使用array_merge()合并；
 * 2）当$outkey指定时，以$outkey值作为键，找到的作为值
 * @param array	    $arr1	【必须】第一个数组
 * [
 *      [
 *          'order_id' => '1',
 *          'goods_id' => '1',
 ...
 *      ]
 * ]
 * @param string    $key	【必须】输入键：合并的判断标识
 * 'order_id'
 * @param string    $outkey	【可选】输出键，默认为null：决定了合并方式
 * 'goods_info'
 * @return boolean	是否合并成功, true：合并成功；false：合并失败
 * 合并成功后的结果，例如：
 * [
 *      [
 *          'order_id' => '1',
 ...
 * ]
 */
function mixed_merge(&$arr1, &$arr2, $key, $outkey=null){
    ...
    if( count($arr1)==0 || count($arr2)==0 || !isset($arr1[0][$key]) || !isset($arr2[0][$key]) ){
	return false;
    }
    
    // 判断 $outkey， 如果存在，必须为字符串，长度大于0，不等于$key
    if( is_string($outkey) ){
	if( strlen($outkey)==0||$outkey==$key ){
	    return false;
	}
    }else{
	$outkey = null;
	...
	// 如果定义了输出键，则将找到的值，作为数组的一个元素
	if( $outkey!=null ){
	    $item[$outkey] = $_v;
	}else{
	    // 没有定义输出键，则用 array_merge()合并
	    // 这种情况，会出现值覆盖的问题
	    $item = array_merge($item,is_array($_v)?$_v:array());
	}
    }
    return true;
}
 ...
 * @return array|false
 */
function replace_field( array $where, array $fieldMap ){
    $resultWhere = array();
    foreach( $where as $k => $v ){
	if( is_array($v) ){
	    $v = replace_field($v, $fieldMap);
	}
	// 处理键类型
	if( is_string($k)){
	    // 替换 字段
	    if( isset($fieldMap[$k]) && !empty($fieldMap[$k]) ){// 定义并且存在时，替换
		$k = $fieldMap[$k];
	    }
	}
	$resultWhere[$k] = $v;
 ...
 * 转换成显示结果行的数组
 * @param array     $arr        数组
 * @param int       $level      缩进深度
 * @param boolen    $last       是否最后
 * @return string
 */
function show_array($arr,$level=0, $last=true){
    $result_arr = array();
    if(is_array($arr) ){
        if( is_assoc_arr($arr) ){
            $str = '';
            if( $level == 0 ){
                $str .= '<div class="line">';
                $str .= '<code class="spaces">'. str_repeat('&nbsp;&nbsp;', $level) .'</code>';
                $str .= '<code class="plain">{</code>';
                $str .= '</div>';
                $result_arr[] = $str;
            }

            $_arr = show_assoc_arr( $arr, $level+1 );

            $result_arr = array_merge( $result_arr, $_arr );

            if( $level == 0 ){
                $str = '<div class="line">';
                $str .= '<code class="spaces">'. str_repeat('&nbsp;&nbsp;', $level) .'</code>';
                $str .= '<code class="plain">}</code>'.($last?'':',');
                $str .= '</div>';
                $result_arr[] = $str;
            }
            ...
            $_arr = show_indexed_arr( $arr, $level+1 );
            $result_arr = array_merge( $result_arr, $_arr );
...
// 处理索引数组
function show_indexed_arr($data,$level=0){
    $arr = array();
    $i = 1;
    $count = count($data);
    foreach( $data as $v ){
        ...
        if( is_string($v) ) {
            $str = '<div class="line">';
            $str .= '<code class="spaces">'. str_repeat('&nbsp;&nbsp;', $level+1) .'</code>';
            $str .= '<code class="string">"'. json_str_escape($v) .'"</code>'.( $last?'':',' );
            $str .= '</div>';
            $arr[] = $str;
        }
        elseif( is_int($v) ){
            $str = '<div class="line">';
            $str .= '<code class="spaces">'. str_repeat('&nbsp;&nbsp;', $level+1) .'</code>';
            $str .= '<code class="json_int">'. $v .'</code>'.( $last?'':',' );
            $str .= '</div>';
            $arr[] = $str;
        }if( is_array($v) ){
            if( count($v) ){
                if(is_assoc_arr($v)){   // 数组中的每个值为关联数组时，新建一行显示
                    $str = '<div class="line">';
                    $str .= '<code class="spaces">'. str_repeat('&nbsp;&nbsp;', $level) .'</code>';
                    $str .= '<code class="plain">{</code>';
                    $str .= '</div>';
                    $arr[] = $str;
                    $_arr = show_assoc_arr($v,$level+1);
                    ...
                    $_arr = show_indexed_arr($v,$level+1);
                    $arr = array_merge( $arr, $_arr );
                }
            }else{
                $str = '<div class="line">';
...
// 处理关联索引
function show_assoc_arr($data,$level=0){
    $arr = array();
    $i = 1;
    $count = count($data);
    foreach( $data as $k => $v ){
        ...
        $str = '<div class="line">';
        $str .= '<code class="spaces">'. str_repeat('&nbsp;&nbsp;', $level) .'</code>';
        $str .= '<code class="json_key">"'. json_str_escape($k) .'"</code>';
        $str .= '<code class="plain">:</code>';
        if( is_null($v) ) {
            $str .= '<code class="null">null</code>'.( $last?'':',' );
            $arr[] = $str;
        }elseif( is_string($v) ) {
            $str .= '<code class="string">"'. json_str_escape($v) .'"</code>'.( $last?'':',' );
            $str .= '</div>';
            ...
            if( count($v) ){    // 第二维，是否存在元素
                if(is_assoc_arr($v)){   // 第二维数组中每一个值都是一个关联数组
                    $str .= '<code class="plain">{</code>';
                    $str .= '</div>';
                    $arr[] = $str;
                    $_arr = show_assoc_arr($v,$level+1);
...
function dialog_result($code,$msg){
...
//弹框输出字符串居中显示并停止程序
function div($value){
    $str = "<div style='width: 100%;height: 100%;display: flex;justify-content: center;align-items: center;margin: 0;padding: 0;' >".$value."</div>";
    return $str;
}

//弹框输出字符串居中显示并停止程序
function echo_div($value){
    $str = "<div style='width: 100%;height: 100%;display: flex;justify-content: center;align-items: center;margin: 0;padding: 0;' >".$value."</div>";
    echo $str;exit;
}
 ...
 *      $encoding   编码
 */
function intercept_string($str,$num=20,$encoding='utf-8'){
    if(mb_strlen( $str, $encoding ) > $num){
        $str = mb_substr($str,0,$num,$encoding) . '...';
    }
    return $str;
}
...
/**
 * 检测扩展是否存在
 * @staticvar type $loaded_extension
 * @param type $name
 * @return boolean
 */
function check_extensions($name){
	static $loaded_extension = null;
	if($loaded_extension==null){
		$loaded_extension = get_loaded_extensions();
	}
	if(in_array($name, $loaded_extension)){
		return true;
	}
	return false;
...
function get_order_no($order_id){
    ...
    $order_info =  model('order2/order','service')->get_order_info(['order_id'=>$order_id]);
 ...
 * return string
 */
function get_order_id($order_no){
    if($order_no < 0){
        return "";
    }

    $order_info =  model('order2/order','service')->get_order_info(['order_no'=>$order_no]);
 ...
 */
function get_age_by_cert_no($id){
 ...
 */
function get_sex_by_cert_no($idcard) {
 ...
 *  根据地址ID 查询省市区详细地址
 *  author:maxiaoyu
 *  @param int $district_id    地址id
 *  @return string $arr 详细地址 例 北京-北京-朝阳
 */
function getDistrict($district_id, $arr = []){
    $menu = model('admin/district','service')->get_info($district_id);
    if($menu['parent_id'] != 0){
        $arr[]  = $menu['name'];
        return getDistrict($menu['parent_id'], $arr); 
    }
...
/**
 * 读取csv   文件内容
 * @param   $csv    string 要读取的文件路径
 * @return  $lists  array  返回二维数组
 */
